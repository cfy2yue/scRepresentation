"""
NicheNet graph loading and per-dataset subgraph construction.

Builds a sparse ``edge_index`` for a given set of genes, including
bidirectional CLS <-> gene edges.
"""
...
class NicheNetGraph:
    """Loads NicheNet graph, filters to vocabulary, and provides fast
    subgraph extraction via a prebuilt token-indexed adjacency dict."""
    ...
    def __init__(self, graph_pkl_path: str, vocab: GeneVocab):
        ...
        max_tok = max(vocab.gene2token.values(), default=0)
        if max_tok >= 40002:
            raise ValueError(
                f"NicheNetGraph: max vocabulary token index {max_tok} >= 40002 "
                f"(see token_to_local buffer in build_edge_index)"
            )
