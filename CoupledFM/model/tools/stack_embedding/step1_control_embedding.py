#!/usr/bin/env python3
"""Encode control cells with Stack → ``biFlow_data/control_stack/{ds}.h5ad``."""
...
LOGGER = logging.getLogger("stack_embed.control")
...
def _run_one(
    ...
    *,
    ...
    out_dir = biflow_dir / "control_stack"
    ...
    t0 = perf_counter()
    LOGGER.info("start dataset=%s mode=control source=%s", dataset_stem, src)
    ...
    try:
        ...
        if n_keep == 0:
            warnings.warn(f"{dataset_stem}: zero control cells after masks; skipping", stacklevel=2)
            return
    ...
    if sub.n_obs == 0:
        warnings.warn(f"{dataset_stem}: zero control cells after mask; skipping", stacklevel=2)
        return
    ...
    try:
        ...
        LOGGER.info("embedding dataset=%s mode=control temp_h5ad=%s", dataset_stem, tmp_path)
    ...
    LOGGER.info(
        "done dataset=%s mode=control cells=%d emb=%s out=%s elapsed=%.1fs",
        dataset_stem,
        sub.n_obs,
        emb.shape,
        out_path,
...
def main() -> None:
    ...
    parser.add_argument("--gene-name-col", default=None, help="optional ``adata.var`` column for symbols")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="write logs here (default: logs/stack_embedding/control_<UTCstamp>/)",
    )
    parser.add_argument(
        "--console",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="temp directory for Stack/anndata (default: <repo>/tmp/stack_embedding on /data2; avoids $HOME/small root /tmp)",
    )
    parser.add_argument(
        "--gpus",
        default="0",
        help="comma-separated GPU ids; datasets are partitioned round-robin across workers",
    )
    parser.add_argument("--datasets", nargs="*", default=None, help="subset of dataset stems")
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("-j", "--jobs", type=int, default=1, help="parallel processes (≤ number of GPUs used)")
    ...
    log_root = (
        ...
        else (coupled_fm_root() / "logs" / "stack_embedding" / f"control_{stamp}")
    )
    ...
    if args.dry_run:
        LOGGER.info(
            "dry_run mode=control log_root=%s tmp_dir=%s datasets=%d",
            log_root,
            resolved_tmp,
            len(names),
        )
    ...
    LOGGER.info(
        "plan mode=control datasets=%d gpus=%s jobs=%d batch_size=%d num_workers=%d progress=%s log_root=%s main_log=%s",
        len(names),
