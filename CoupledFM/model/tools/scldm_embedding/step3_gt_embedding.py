#!/usr/bin/env python3
"""Encode non-control (GT) cells with scLDM → ``latent_data/scldm/gt_scldm/{ds}.h5ad``."""
...
LOGGER = logging.getLogger("scldm_embed.gt")
...
def _run_one(
    ...
    *,
    ...
    t0 = perf_counter()
    LOGGER.info("start dataset=%s mode=gt_scldm source=%s", dataset_stem, src)
    ...
    LOGGER.info(
        "done dataset=%s mode=gt cells=%d out=%s elapsed=%.1fs",
        dataset_stem,
        sub.n_obs,
        out_path,
        perf_counter() - t0,
...
def main() -> None:
    ...
    log_root = (
        ...
        else (coupled_fm_root() / "logs" / "scldm_embedding" / f"gt_{stamp}")
    )
    ...
    if args.dry_run:
        LOGGER.info(
            "dry_run mode=gt latent_root=%s tmp=%s datasets=%d",
            args.latent_root,
            resolved_tmp,
            len(names),
        )
    ...
    LOGGER.info(
        "plan mode=gt_scldm datasets=%d gpus=%s jobs=%d batch=%d latent_root=%s log=%s",
        len(names),
