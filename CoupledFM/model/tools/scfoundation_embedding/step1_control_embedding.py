#!/usr/bin/env python3
"""Encode control cells with scFoundation → ``latent_data/scfoundation/control_scfoundation/{ds}.h5ad``."""
...
LOGGER = logging.getLogger("scfoundation_embed.control")
...
def _run_one(
    ...
    *,
    ...
    out_dir = out_root / "control_scfoundation"
    shard_dir = out_root / "_shards" / "control_scfoundation" / dataset_stem
    ...
    t0 = perf_counter()
    LOGGER.info(
        "start dataset=%s mode=control_scfoundation shard=%d/%d selected_range=%d:%d source=%s",
        dataset_stem,
        shard_index + 1,
        shard_count,
        selected_start,
    ...
    n_keep = int(keep.sum())
    LOGGER.info(
        "selected dataset=%s mode=control_scfoundation cells=%d/%d",
        dataset_stem,
        n_keep,
        adata.n_obs,
    )
    ...
    mmap_path = tmp_dir / f"scfoundation_{dataset_stem}_{os.getpid()}_{id(sub)}.emb.memmap"
    ...
    LOGGER.info(
        "done dataset=%s mode=control_scfoundation cells=%d out=%s elapsed=%.1fs",
        dataset_stem,
        sub.n_obs,
        out_path,
        perf_counter() - t0,
...
def main() -> None:
    ...
    log_root = (
        ...
        else default_log_dir(coupled_fm_root(), "control")
    )
    ...
    if args.dry_run:
        LOGGER.info(
            "dry_run mode=control_scfoundation latent_root=%s tmp=%s datasets=%d schedule=%s target_cells_per_shard=%s",
            args.latent_root,
            resolved_tmp,
            len(names),
            args.schedule,
        ...
        for ds in sorted(names):
            ...
            LOGGER.info(
                "dry_run dataset=%s raw_path=%s raw_n_obs_est=%d selected_control_est=%d",
                ds,
                rp,
    ...
    if not args.overwrite:
        final_dir = latent_out / "control_scfoundation"
        before = len(names)
        names = [ds for ds in names if not (final_dir / f"{ds}.h5ad").is_file()]
        skipped = before - len(names)
        if skipped:
            LOGGER.info("skip_existing_final_outputs mode=control_scfoundation skipped=%d remaining=%d", skipped, len(names))
        if not names:
            LOGGER.info("all control_scfoundation outputs already exist; nothing to do")
            return
    ...
    LOGGER.info(
        "plan mode=control_scfoundation datasets=%d gpus=%s jobs=%d chunk=%s latent_root=%s log=%s schedule=%s",
        len(names),
