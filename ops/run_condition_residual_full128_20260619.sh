...
RUN_ROOT=${ROOT}/runs/latentfm_condition_residual_full128_20260619
REPORT_ROOT=${ROOT}/reports/latentfm_condition_residual_full128_20260619

mkdir -p "${RUN_ROOT}/logs" "${REPORT_ROOT}"

cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# LatentFM Condition Residual Full128 Audit

Started: $(date '+%F %T %Z')
Status: running
Purpose: full canonical-test residual/ranking audit with 128 sampled cells per condition.
...
run_one primary_scfoundation 0 "${COUPLED}/output/latentfm_runs/full_scfoundation/20260617_scfoundation_comp006_delta_w5_12k/best.pt" &
...
if [[ "${status}" == "0" ]]; then
  "${PY}" "${ROOT}/ops/summarize_condition_residual_audit.py" \
    ...
    --out-md "${ROOT}/reports/LATENTFM_CONDITION_RESIDUAL_FULL128_20260619.md" \
    --out-json "${ROOT}/reports/latentfm_condition_residual_full128_20260619.json" \
...
Outputs: ${REPORT_ROOT}
Report: ${ROOT}/reports/LATENTFM_CONDITION_RESIDUAL_FULL128_20260619.md
EOF
