...
if [[ "${LATENTFM_GENERAL_EXPOSURE_MMDGUARD_ACK:-}" != "repair_not_blind_rerun" ]]; then
  echo "Set LATENTFM_GENERAL_EXPOSURE_MMDGUARD_ACK=repair_not_blind_rerun" >&2
  exit 4
fi

RUN_ROOT=${ROOT}/runs/latentfm_general_exposure_mmdguard_repair_20260624
OUT_ROOT=${COUPLED}/output/latentfm_runs/general_exposure_mmdguard_repair_20260624
LOG_ROOT=${ROOT}/logs/latentfm_general_exposure_mmdguard_repair_20260624
...
SUMMARIZER=${ROOT}/ops/summarize_latentfm_general_exposure_mmdguard_repair_20260624.py

mkdir -p "${RUN_ROOT}/logs" "${OUT_ROOT}" "${LOG_ROOT}" "${ROOT}/reports"

for required in "${DATA_DIR}/manifest.json" "${SPLIT_FILE}" "${PERT_MEANS}" "${ANCHOR_CKPT}" "${GENE_CACHE}/manifest.json" "${GPU_HELPER}" "${TRAIN_LAUNCHER}" "${SUMMARIZER}" "${ROOT}/reports/LATENTFM_XVERSE_GENERAL_EXPOSURE_FAILURE_CASES_20260624.md"; do
  [[ -e "${required}" ]] || { echo "Missing required artifact: ${required}" >&2; exit 2; }
done

declare -a RUN_NAMES=(
  "xverse_general_exposure_mmdguard_replay05_mmd05_3k_seed42"
)
declare -a REPLAY_WEIGHTS=(0.5)
need=${#RUN_NAMES[@]}
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_GENERAL_EXPOSURE_MMDGUARD_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_GENERAL_EXPOSURE_MMDGUARD_RERUN=1" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before general exposure MMD-guard launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
reasons=[]
if len(suggested)<need: reasons.append(f"only {len(suggested)} GPU slots suggested for need={need}")
if float(system.get("mem_available_gib") or 0)<128: reasons.append("low_mem")
if float(system.get("load1_per_cpu") or 0)>2: reasons.append("high_cpu_load")
if reasons: audit["status"]="fail"; audit["reasons"]=reasons
...
import json, sys
from pathlib import Path
for gpu in json.loads(Path(sys.argv[1]).read_text())["assigned_gpus"]:
    print(int(gpu))
PY
)
...
General exposure-cap v2 had small Pearson gains but failed from family MMD harm. This repair adds stronger MMD pressure and anchor replay to test whether the Pearson signal survives while worst-dataset MMD harm collapses.

## Command

\`\`\`bash
LATENTFM_GENERAL_EXPOSURE_MMDGUARD_ACK=repair_not_blind_rerun bash ${ROOT}/ops/launch_latentfm_general_exposure_mmdguard_repair_20260624.sh
\`\`\`
...
* \`${ROOT}/reports/LATENTFM_GENERAL_EXPOSURE_MMDGUARD_REPAIR_DECISION_20260624.md\`
...
- Anchor replay weight: ${replay}; gamma 0.05; mmd_every 1; selection pearson_pert_minus_mmd lambda 1.0.
- Split: \`${SPLIT_FILE}\`.
- Stop if family MMD or worst-dataset MMD harm persists.
...
# Run Status: latentfm_general_exposure_mmdguard_repair_20260624
...
Started ${need} general exposure MMD-guarded repair smokes.

## Notes

- Repair branch motivated by:
  \`${ROOT}/reports/LATENTFM_XVERSE_GENERAL_EXPOSURE_FAILURE_CASES_20260624.md\`
- Primary arm follows external audit recommendation: anchor replay 0.5, gamma 0.05, MMD every step.
- Internal gate only; no canonical multi/Track C query.
EOF
