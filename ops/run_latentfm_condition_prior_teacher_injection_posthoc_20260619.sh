...
TRAIN_RUN_ROOT=${ROOT}/runs/latentfm_condition_prior_teacher_injection_20260619
RUN_ROOT=${ROOT}/runs/latentfm_condition_prior_teacher_injection_posthoc_20260619
RUN_DIR=${COUPLED}/output/latentfm_runs/condition_prior_teacher_injection_20260619/scf_prior010_inject_e2_4k
...
# LatentFM Condition-Prior Injection Posthoc 2026-06-19
...
while true; do
  ...
  if ! tmux has-session -t latentfm_condition_prior_teacher_injection_20260619 2>/dev/null; then
    cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# LatentFM Condition-Prior Injection Posthoc 2026-06-19
...
posthoc_log="${RUN_ROOT}/logs/scf_prior010_inject_e2_4k.posthoc.log"
posthoc_exit="${RUN_ROOT}/logs/scf_prior010_inject_e2_4k.posthoc.exit"

{
  echo "[$(date +%F_%T)] start injection posthoc gpu=${gpu}"
