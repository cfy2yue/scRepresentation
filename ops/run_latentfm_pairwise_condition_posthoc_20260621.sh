...
RUN_ROOT=${LATENTFM_PAIRWISE_RUN_ROOT:-${ROOT}/runs/latentfm_pairwise_condition_20260621}
OUT_ROOT=${LATENTFM_PAIRWISE_OUT_ROOT:-${COUPLED}/output/latentfm_runs/pairwise_condition_20260621}
...
SUMMARY=${ROOT}/ops/summarize_latentfm_pairwise_condition_smoke_20260621.py
...
RUN_NAME=${LATENTFM_PAIRWISE_RUN_NAME:-scf_pair_hadamard_prior010_inject_e2_4k}
SUMMARY_JSON=${LATENTFM_PAIRWISE_SUMMARY_JSON:-${ROOT}/reports/latentfm_pairwise_condition_smoke_summary_20260621.json}
SUMMARY_MD=${LATENTFM_PAIRWISE_SUMMARY_MD:-${ROOT}/reports/LATENTFM_PAIRWISE_CONDITION_SMOKE_SUMMARY_20260621.md}
BOOTSTRAP_DIR=${LATENTFM_PAIRWISE_BOOTSTRAP_DIR:-${ROOT}/reports/latentfm_pairwise_condition_smoke_bootstrap_20260621}
POSTHOC_TITLE=${LATENTFM_PAIRWISE_POSTHOC_TITLE:-latentfm_pairwise_condition_posthoc_20260621}
...
\`\`\`bash
bash ${ROOT}/ops/run_latentfm_pairwise_condition_posthoc_20260621.sh
\`\`\`
...
Waiting for pairwise-condition smoke training to finish.
EOF

trap 'rc=$?; echo "$rc" > "${RUN_ROOT}/POSTHOC_EXIT_CODE"; date "+%F %T %Z" > "${RUN_ROOT}/POSTHOC_FINISHED"; exit "$rc"' EXIT

{
  echo "[$(date '+%F %T %Z')] pairwise condition posthoc watcher start"
  ...
  echo "[$(date '+%F %T %Z')] exact GPU status before pairwise posthoc"
...
PY
  ...
  "${PYTHON}" "${BOOTSTRAP_RUNNER}" \
    ...
    --n-boot 2000 \
    --seed 42
  echo "[$(date '+%F %T %Z')] pairwise condition posthoc done"
