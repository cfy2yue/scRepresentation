...
TRAIN_RUN_ROOT=${ROOT}/runs/latentfm_corrected_dsloss_smoke_20260619
RUN_ROOT=${ROOT}/runs/latentfm_corrected_dsloss_posthoc_20260619
TRAIN_OUT_ROOT=${COUPLED}/output/latentfm_runs/corrected_dsloss_smoke_20260619
...
# Run Status: latentfm_corrected_dsloss_posthoc_20260619

## Command

\`\`\`bash
bash ${ROOT}/ops/run_latentfm_corrected_dsloss_posthoc_after_finish_20260619.sh
\`\`\`
...
tmux: \`latentfm_corrected_dsloss_posthoc_20260619\`
...
* \`${TRAIN_OUT_ROOT}/${RUN_NAME}/posthoc_eval/split_group_eval_best_ode20_mse1024_mmd1024_stablecaps.json\`
* \`${TRAIN_OUT_ROOT}/${RUN_NAME}/posthoc_eval/condition_family_eval_best_ode20_mse1024_mmd1024_stablecaps.json\`
* \`${ROOT}/reports/LATENTFM_CORRECTED_DSLOSS_STABLECAPS_SUMMARY_20260619.md\`
...
Waiting for corrected dsloss training EXIT_CODE with \`sleep 1800\` cadence.
EOF

log=${RUN_ROOT}/logs/run.log
{
  echo "[$(date '+%F %T %Z')] wait for corrected dsloss training exit"
  ...
  echo "[$(date '+%F %T %Z')] exact GPU status before corrected dsloss posthoc"
...
PY
  ...
  "${PYTHON}" "${ROOT}/ops/audit_latentfm_stablecaps_selection.py" \
    ...
    --out-json "${ROOT}/reports/latentfm_stablecaps_selection_audit_${RUN_NAME}_20260619.json" \
    --out-md "${ROOT}/reports/LATENTFM_STABLECAPS_SELECTION_AUDIT_${RUN_NAME}_20260619.md"

  "${PYTHON}" "${ROOT}/ops/summarize_latentfm_sampling_smokes_stablecaps_20260619.py" \
    --baseline-name scf_prior010_inject_e2_4k \
    --baseline-dir "${BASELINE_DIR}" \
    --run "${RUN_NAME}" "${TRAIN_OUT_ROOT}/${RUN_NAME}" \
    --out-csv "${ROOT}/reports/latentfm_corrected_dsloss_stablecaps_summary_20260619.csv" \
    --out-md "${ROOT}/reports/LATENTFM_CORRECTED_DSLOSS_STABLECAPS_SUMMARY_20260619.md"

  echo "[$(date '+%F %T %Z')] finished corrected dsloss posthoc"
} > "${log}" 2>&1
