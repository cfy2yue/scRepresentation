...
RUN_ROOT=${ROOT}/runs/latentfm_xverse_trackc_endpoint_parallel_b_20260622
OUT_ROOT=${ROOT}/CoupledFM/output/latentfm_runs/xverse_trackc_endpoint_parallel_b_20260622
LOG_ROOT=${ROOT}/logs/latentfm_xverse_trackc_endpoint_parallel_b_20260622
...
MANIFEST=${ROOT}/reports/latentfm_trackc_endpoint_parallel_smokes_b_manifest_20260622.jsonl
...
row = {
    ...
    "run_status": f"/data/cyx/1030/scLatent/runs/latentfm_xverse_trackc_endpoint_parallel_b_20260622/{sys.argv[2]}/RUN_STATUS.md",
...
launch_one \
  xverse_trackc_endpoint_w010_replay1_2k_seed42 \
  0.10 0 500 0.0 \
  "Very low endpoint-routed teacher weight tests whether any support signal can be gained without canonical family_gene pp harm."

sleep 20

launch_one \
  xverse_trackc_endpoint_w075_replay1_2k_seed42 \
  0.75 0 500 0.0 \
  "Intermediate endpoint-routed teacher weight tests whether support gain increases between failed w0.5 and high w1.0 variants."

sleep 20

launch_one \
  xverse_trackc_endpoint_w150_replay1_2k_seed42 \
  1.50 0 500 0.0 \
  "Aggressive endpoint-routed teacher weight tests whether support route capture needs stronger endpoint pressure, with canonical no-harm as veto."

sleep 20

launch_one \
  xverse_trackc_endpoint_w100_delayed500_1500_replay1_2k_seed42 \
  1.00 500 1500 0.0 \
  "Delayed high endpoint-routed supervision tests whether anchor replay warmup reduces canonical no-harm risk while keeping stronger support pressure."

echo "parallel_endpoint_smokes_b_launched"
