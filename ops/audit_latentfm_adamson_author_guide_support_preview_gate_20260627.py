#!/usr/bin/env python3
"""Single-source preview for Adamson author guide-support artifacts.

The artifacts are guide/read/UMI/coverage support metadata, so this preview is
diagnostic-only and cannot authorize GPU even if association is present.
"""
...
ARTIFACT_CSV = ROOT / "reports/adamson_author_guide_support_artifacts_20260627/adamson_author_guide_support_artifacts.csv"
OUT_DIR = ROOT / "reports/adamson_author_guide_support_preview_gate_20260627"
OUT_JOINED = OUT_DIR / "adamson_author_guide_support_preview_joined_rows.csv"
OUT_SUMMARY = OUT_DIR / "adamson_author_guide_support_preview_summary.csv"
OUT_JSON = ROOT / "reports/latentfm_adamson_author_guide_support_preview_gate_20260627.json"
OUT_MD = ROOT / "reports/LATENTFM_ADAMSON_AUTHOR_GUIDE_SUPPORT_PREVIEW_GATE_20260627.md"
DATASET = "Adamson"
...
def summarize(rows: list[dict[str, Any]], artifact: str) -> dict[str, Any]:
    ...
    reasons = ["qc_reagent_support_duplicate_no_gpu", "single_source_test_metric_preview_only"]
...
def main() -> int:
    ...
    status = "adamson_author_guide_support_preview_diagnostic_no_gpu"
    ...
    lines = [
        "# LatentFM Adamson Author Guide-Support Preview Gate 2026-06-27",
        "",
        f"Status: `{status}`",
        "",
        ...
        "## Boundary",
        "",
        "- CPU/report-only single-source diagnostic preview over Adamson author guide-support artifacts.",
        "- Uses frozen seed42/43 xverse Adamson `test_single` rows only; no canonical multi, Track C query, training, inference, or GPU.",
        "- These artifacts are guide/read/UMI/coverage support metadata, so they cannot authorize GPU even if associated.",
        "",
        "## Summary",
        "",
