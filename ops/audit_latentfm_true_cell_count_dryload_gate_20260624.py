#!/usr/bin/env python3
"""Dry-load gate for true cell-count capped-H5 LatentFM artifacts."""
...
OUT_JSON = ROOT / "reports/latentfm_true_cell_count_dryload_gate_20260624.json"
OUT_MD = ROOT / "reports/LATENTFM_TRUE_CELL_COUNT_DRYLOAD_GATE_20260624.md"
...
def dryload_row(row: dict[str, Any], *, max_batches: int) -> dict[str, Any]:
    ...
    for path in [data_dir / "manifest.json", data_dir / "pert_means.npz", data_dir / "ctrl_means.npz", split_file, GENE_CACHE / "manifest.json"]:
        if not path.exists():
    ...
    results = {}
    for mode, min_cells in [("train", 32), ("test", 16)]:
        ...
        for _ in range(max_batches):
            ...
            if len(batch) == 5 and batch[4] is None:
                reasons.append(f"{mode}_missing_perturbation_batch")
            batches_checked += 1
        results[mode] = {
...
def render_md(payload: dict[str, Any]) -> str:
    lines = [
        "# LatentFM True Cell-Count Dry-Load Gate",
        "",
        f"Status: `{payload['status']}`",
        "",
        "## Boundary",
        "",
        "- CPU-only loader compatibility gate.",
        "- Instantiates `CrossDatasetFMDataset` on capped `DATA_DIR`, matching split, and gene perturbation cache.",
        "- Reads a few train/test batches only; does not train, infer, read canonical multi, or use Track C query.",
        "",
        "## Rows",
        "",
...
def main() -> int:
    ...
    if reasons:
        status = "true_cell_count_dryload_not_ready_no_gpu"
        next_action = "run after materialization, sample provenance, and schema gate pass"
    elif all(row.get("status") == "ok" for row in audit_rows):
        status = "true_cell_count_dryload_pass_no_gpu"
        next_action = "run design controls, then consider one bounded exploratory smoke after resource audit"
    else:
        status = "true_cell_count_dryload_fail_no_gpu"
