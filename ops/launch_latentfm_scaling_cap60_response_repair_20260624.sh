...
if [[ "${LATENTFM_CAP60_RESPONSE_REPAIR_ACK:-}" != "cap60_trainonly_normalizer_ready" ]]; then
  echo "Set LATENTFM_CAP60_RESPONSE_REPAIR_ACK=cap60_trainonly_normalizer_ready" >&2
  exit 4
fi

RUN_ROOT=${ROOT}/runs/latentfm_scaling_cap60_response_repair_20260624
OUT_ROOT=${COUPLED}/output/latentfm_runs/scaling_cap60_response_repair_20260624
LOG_ROOT=${ROOT}/logs/latentfm_scaling_cap60_response_repair_20260624
...
SUMMARIZER=${ROOT}/ops/summarize_latentfm_scaling_cap60_response_repair_20260624.py
...
declare -a RUN_NAMES=(
  "xverse_scaling_cap60_resp010_replay05_4k_seed42"
  "xverse_scaling_cap60_resp025_replay05_4k_seed42"
)
declare -a RESPONSE_WEIGHTS=(0.10 0.25)
need=${#RUN_NAMES[@]}
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_CAP60_RESPONSE_REPAIR_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_CAP60_RESPONSE_REPAIR_RERUN=1 to relaunch" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before cap60 response repair launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
Cap60-specific train-only response normalization plus light anchor replay may keep the seed42 cap60 internal cross/family signal while reducing MMD/no-harm drift.

## Command

\`\`\`bash
LATENTFM_CAP60_RESPONSE_REPAIR_ACK=cap60_trainonly_normalizer_ready bash ${ROOT}/ops/launch_latentfm_scaling_cap60_response_repair_20260624.sh
\`\`\`
...
* \`${ROOT}/reports/LATENTFM_SCALING_CAP60_RESPONSE_REPAIR_DECISION_20260624.md\`
...
- Response weight: ${weight}; anchor replay weight: 0.5.
- Response normalizer: \`${NORMALIZER}\`.
- Train split: \`${SPLIT_FILE}\`.
- Canonical multi and held-out Track C query are not used.
EOF
  echo "Launched ${run_name} on GPU ${gpu} in tmux ${session}"
done

cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# Run Status: latentfm_scaling_cap60_response_repair_20260624
...
Started ${need} cap60 response-normalized repair smokes.

## Notes

- Uses cap60-specific train-only normalizer:
  \`${NORMALIZER}\`
