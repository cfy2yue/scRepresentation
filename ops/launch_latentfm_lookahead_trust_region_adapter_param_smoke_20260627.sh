...
MAX_ACCEPTED="${MAX_ACCEPTED:-40}"
STEP_GRID="${STEP_GRID:-1,3,10,30,100,300}"
ANCHOR_THRESHOLD="${ANCHOR_THRESHOLD:-1e-6}"
MIN_TASK_DELTA="${MIN_TASK_DELTA:-1e-10}"
MIN_FOOTPRINT="${MIN_FOOTPRINT:-1e-7}"

DEFAULT_NAME="xverse_lookahead_trust_region_adapter_seed${SEED}_${MAX_ACCEPTED}accepted_$(date +%Y%m%d_%H%M%S)"
...
projected updates can make train-task progress while preserving
anchor-replay/no-harm behavior. This parameterized launcher is for bounded
replication or extension after the fixed seed42/40-accepted smoke reaches its
gate.

## Command

\`\`\`bash
SEED=$SEED BATCH_SIZE=$BATCH_SIZE MAX_ATTEMPTS=$MAX_ATTEMPTS MAX_ACCEPTED=$MAX_ACCEPTED STEP_GRID='$STEP_GRID' ANCHOR_THRESHOLD=$ANCHOR_THRESHOLD MIN_TASK_DELTA=$MIN_TASK_DELTA MIN_FOOTPRINT=$MIN_FOOTPRINT $0 $GPU_ID $RUN_NAME
...
frozen and must use a separate canonical no-harm launcher.
EOF
