...
OUT_ROOT=${ROOT}/CoupledFM/output/latentfm_runs/xverse_smoke_20260620
...
REPORT=${REPORT:-"${ROOT}/reports/LATENTFM_XVERSE_SMOKE_ONE_SHOT_STATUS_$(date +%Y%m%d_%H%M%S).txt"}
...
{
  echo "[$(date '+%F %T %Z')] one-shot LatentFM xverse smoke status check"
  echo "report=${REPORT}"
  echo
  ...
  if [[ -f "${LOG_ROOT}/${RUN_NAME}.log" ]]; then
    ...
    tail -n 100 "${LOG_ROOT}/${RUN_NAME}.log"
  else
