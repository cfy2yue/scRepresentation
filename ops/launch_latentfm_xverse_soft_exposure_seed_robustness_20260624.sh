...
if [[ "${LATENTFM_XVERSE_SOFT_SEED_ACK:-}" != "soft_p085_canonical_noharm_pass" ]]; then
  cat >&2 <<'EOF'
Refusing to launch xverse soft-exposure seed robustness.

Set:
  LATENTFM_XVERSE_SOFT_SEED_ACK=soft_p085_canonical_noharm_pass
...
CANON_JSON=${ROOT}/reports/latentfm_xverse_soft_exposure_canonical_noharm_decision_20260624.json
if [[ ! -e "${CANON_JSON}" ]]; then
  echo "Missing soft canonical no-harm decision JSON: ${CANON_JSON}" >&2
  exit 2
fi
...
if decision.get("status") != "soft_exposure_canonical_noharm_pass":
    raise SystemExit(f"soft canonical no-harm did not pass: {decision.get('status')!r}")
if "xverse_softvisit_p085_no_cap_3k_seed42" not in passed:
    raise SystemExit(f"seed42 p085 not in passed_runs: {sorted(passed)}")
PY

RUN_ROOT=${LATENTFM_XVERSE_SOFT_SEED_RUN_ROOT:-${ROOT}/runs/latentfm_xverse_soft_exposure_seed_robustness_20260624}
OUT_ROOT=${LATENTFM_XVERSE_SOFT_SEED_OUT_ROOT:-${COUPLED}/output/latentfm_runs/xverse_soft_exposure_seed_robustness_20260624}
LOG_ROOT=${LATENTFM_XVERSE_SOFT_SEED_LOG_ROOT:-${ROOT}/logs/latentfm_xverse_soft_exposure_seed_robustness_20260624}
...
SUMMARIZER=${ROOT}/ops/summarize_latentfm_xverse_soft_exposure_seed_robustness_20260624.py

mkdir -p "${RUN_ROOT}/logs" "${OUT_ROOT}" "${LOG_ROOT}" "${ROOT}/reports"

SEEDS_STR=${LATENTFM_XVERSE_SOFT_SEEDS:-"43 44"}
...
for seed in "${SEEDS[@]}"; do
  ...
  run_name="xverse_softvisit_p085_no_cap_3k_seed${seed}"
  RUN_NAMES+=("${run_name}")
  run_dir=${RUN_ROOT}/${run_name}
  out_dir=${OUT_ROOT}/${run_name}
  session=lfm_${run_name}
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_XVERSE_SOFT_SEED_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_XVERSE_SOFT_SEED_RERUN=1 to relaunch" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before soft-exposure seed launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
if int(payload.get("max_user_gpus") or 0) > 4:
    reasons.append("max_user_gpus exceeds soft-confirm cap 4")
...
export CONDITION_VISIT_POWER=0.85
...
internal_common=(--data-dir ${DATA_DIR} --biflow-dir ${BIFLOW_DIR} --split-file ${SPLIT_FILE} --pert-means-file ${PERT_MEANS} --gpu 0 --ode-steps 20 --max-chunk 512 --eval-max-conditions 0 --eval-max-conditions-per-dataset 0 --eval-max-mse-cells 1024 --eval-max-mmd-cells 1024)
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${ANCHOR_CKPT} --groups test test_single internal_val_cross_background_seen_gene_proxy internal_val_family_gene_proxy --out "\${internal_dir}/split_group_eval_anchor_internal_ode20.json" "\${internal_common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${ANCHOR_CKPT} --groups test_all family_gene family_drug test_single --out "\${internal_dir}/condition_family_eval_anchor_internal_ode20.json" "\${internal_common[@]}"
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${out_dir}/best.pt --groups test test_single internal_val_cross_background_seen_gene_proxy internal_val_family_gene_proxy --out "\${internal_dir}/split_group_eval_candidate_internal_ode20.json" "\${internal_common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${out_dir}/best.pt --groups test_all family_gene family_drug test_single --out "\${internal_dir}/condition_family_eval_candidate_internal_ode20.json" "\${internal_common[@]}"

eval_dir=${run_dir}/posthoc_eval_canonical
mkdir -p "\${eval_dir}"
canonical_common=(--data-dir ${DATA_DIR} --biflow-dir ${BIFLOW_DIR} --split-file ${CANONICAL_SPLIT} --gpu 0 --ode-steps 20 --max-chunk 512 --eval-max-conditions 0 --eval-max-conditions-per-dataset 0 --eval-max-mse-cells 1024 --eval-max-mmd-cells 1024)
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${ANCHOR_CKPT} --groups test test_single test_multi test_multi_seen test_multi_unseen1 test_multi_unseen2 --out "\${eval_dir}/split_group_eval_anchor_ode20_canonical.json" "\${canonical_common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${ANCHOR_CKPT} --groups test_all family_gene family_drug structure_single structure_multi test_single test_multi test_multi_seen test_multi_unseen1 test_multi_unseen2 --out "\${eval_dir}/condition_family_eval_anchor_ode20_canonical.json" "\${canonical_common[@]}"
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${out_dir}/best.pt --groups test test_single test_multi test_multi_seen test_multi_unseen1 test_multi_unseen2 --out "\${eval_dir}/split_group_eval_candidate_ode20_canonical.json" "\${canonical_common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${out_dir}/best.pt --groups test_all family_gene family_drug structure_single structure_multi test_single test_multi test_multi_seen test_multi_unseen1 test_multi_unseen2 --out "\${eval_dir}/condition_family_eval_candidate_ode20_canonical.json" "\${canonical_common[@]}"
...
${PYTHON} ${ROOT}/ops/summarize_latentfm_single_background_candidate_decision_20260622.py --gate-json "\${eval_dir}/single_background_candidate_gate.json" --label ${run_name} --title "LatentFM xverse soft-exposure seed robustness decision" --out-md "\${eval_dir}/SINGLE_BACKGROUND_CANDIDATE_DECISION.md"
${PYTHON} ${SUMMARIZER}
EOF
...
Fixed p085 no-hardcap soft exposure should reproduce the seed42 canonical
no-harm signal under a new random seed without using canonical metrics for
hyperparameter selection.

## Command

\`\`\`bash
LATENTFM_XVERSE_SOFT_SEED_ACK=soft_p085_canonical_noharm_pass bash ${ROOT}/ops/launch_latentfm_xverse_soft_exposure_seed_robustness_20260624.sh
\`\`\`

## Runtime classification

Long GPU training plus posthoc. Use 30-minute cadence for checks.
...
* \`${ROOT}/reports/LATENTFM_XVERSE_SOFT_EXPOSURE_SEED_ROBUSTNESS_DECISION_20260624.md\`
...
cat ${run_dir}/${run_name}.EXIT_CODE 2>/dev/null || echo "training still running"
...
- Fixed route: `xverse_softvisit_p085_no_cap_3k_seed42` canonical pass required before launch.
- Train split: \`${SPLIT_FILE}\`; canonical split is post-training confirmation only.
- Canonical multi groups remain diagnostic only.
EOF
  echo "Launched soft p085 seed robustness ${run_name} on GPU ${gpu}"
done
