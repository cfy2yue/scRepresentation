...
RUN_ROOT=${ROOT}/runs/latentfm_pairwise_condition_20260621
OUT_ROOT=${COUPLED}/output/latentfm_runs/pairwise_condition_20260621
LOG_ROOT=${ROOT}/logs/latentfm_pairwise_condition_20260621
...
RUN_NAME=scf_pair_hadamard_prior010_inject_e2_4k
...
if [[ "${FORCE_PAIRWISE_CONDITION_RERUN:-0}" != "1" && -e "${OUT_ROOT}/${RUN_NAME}" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_PAIRWISE_CONDITION_RERUN=1 to relaunch" >&2
  exit 3
fi

echo "[$(date '+%F %T %Z')] exact GPU status before pairwise-condition launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
tmux new -d -s latentfm_pairwise_condition_posthoc_20260621 \
  "bash -lc 'bash ${POSTHOC_SCRIPT}'"

cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# Run Status: latentfm_pairwise_condition_20260621

## Command

\`\`\`bash
bash ${ROOT}/ops/launch_latentfm_pairwise_condition_smoke_20260621.sh
\`\`\`
...
Default-off pairwise encoder branch enabled only for this smoke via
\`--pert-pairwise-mode hadamard_mean\`. No held-out labels, response residuals,
posthoc predictions, or combo IDs are used to define pair features. Passing this
4k smoke only permits capped posthoc, uncapped posthoc, and bootstrap; it is not
promotion by itself.
EOF
