...
if [[ "${LATENTFM_XVERSE_OTPAIR_ACK:-}" != "ot_pair_mode_codegate_pass" ]]; then
  cat >&2 <<'EOF'
Refusing to launch xverse OT pair-mode smokes.

Set:
  LATENTFM_XVERSE_OTPAIR_ACK=ot_pair_mode_codegate_pass

Required prereads:
  reports/LATENTFM_OT_PAIRING_SIGNAL_AUDIT_20260624.md
  docs/BUGS_AND_FIXES.md
EOF
  exit 4
fi

RUN_ROOT=${LATENTFM_XVERSE_OTPAIR_RUN_ROOT:-${ROOT}/runs/latentfm_xverse_ot_pairmode_smokes_20260624}
OUT_ROOT=${LATENTFM_XVERSE_OTPAIR_OUT_ROOT:-${COUPLED}/output/latentfm_runs/xverse_ot_pairmode_smokes_20260624}
LOG_ROOT=${LATENTFM_XVERSE_OTPAIR_LOG_ROOT:-${ROOT}/logs/latentfm_xverse_ot_pairmode_smokes_20260624}
...
for required in \
  ...
  "${GPU_HELPER}" \
  "${TRAIN_LAUNCHER}" \
  "${ROOT}/reports/LATENTFM_OT_PAIRING_SIGNAL_AUDIT_20260624.md" \
...
declare -a MODES=("multinomial" "assignment" "random")
declare -a HYPOTHESES=(
  "Matched current-default control: transport-plan multinomial sampling with replacement."
  "Greedy assignment from the same mini-batch Sinkhorn plan preserves one-to-one batch marginals while keeping low-cost paths."
  "No-OT random same-condition pairing tests whether mini-batch OT coupling is actually beneficial."
)

if [[ -n "${LATENTFM_XVERSE_OTPAIR_ONLY_MODE:-}" ]]; then
  case "${LATENTFM_XVERSE_OTPAIR_ONLY_MODE}" in
    multinomial)
      RUN_NAMES=("xverse_otpair_multinomial_2k_seed42")
      MODES=("multinomial")
      HYPOTHESES=("Matched current-default control: transport-plan multinomial sampling with replacement.")
      ;;
    assignment)
      RUN_NAMES=("xverse_otpair_assignment_2k_seed42")
      MODES=("assignment")
      HYPOTHESES=("Greedy assignment from the same mini-batch Sinkhorn plan preserves one-to-one batch marginals while keeping low-cost paths.")
      ;;
    random)
      RUN_NAMES=("xverse_otpair_random_2k_seed42")
      MODES=("random")
      HYPOTHESES=("No-OT random same-condition pairing tests whether mini-batch OT coupling is actually beneficial.")
      ;;
    hungarian)
      MARGINAL_JSON=${ROOT}/reports/latentfm_ot_marginal_preserving_reopen_gate_20260624.json
      RANDOM_JSON=${ROOT}/reports/latentfm_xverse_ot_pairmode_random_control_decision_20260624.json
      if [[ ! -e "${MARGINAL_JSON}" ]]; then
        echo "Missing Hungarian marginal-preserving CPU gate JSON: ${MARGINAL_JSON}" >&2
        exit 2
      fi
      if [[ ! -e "${RANDOM_JSON}" ]]; then
        echo "Missing random/no-OT control decision JSON: ${RANDOM_JSON}" >&2
        exit 2
      fi
      "${PYTHON}" - "${MARGINAL_JSON}" "${RANDOM_JSON}" <<'PY'
import json
import sys
from pathlib import Path

marginal = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
random = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
m_status = (marginal.get("decision") or {}).get("status")
r_status = (random.get("decision") or {}).get("status")
if m_status != "ot_marginal_preserving_reopen_gate_pass_no_gpu":
    raise SystemExit(f"Hungarian marginal-preserving CPU gate not passed: {m_status!r}")
if r_status != "all_done_no_pass":
    raise SystemExit(f"random/no-OT control is not completed no-pass: {r_status!r}")
PY
      RUN_NAMES=("xverse_otpair_hungarian_2k_seed42")
      MODES=("hungarian")
      HYPOTHESES=("Default-off Hungarian OT redesign: true min-cost one-to-one mini-batch assignment passed train-only marginal-preserving CPU gate after random/no-OT control failed; test whether it improves Track A without canonical no-harm violations.")
      ;;
    *)
      echo "Unsupported LATENTFM_XVERSE_OTPAIR_ONLY_MODE=${LATENTFM_XVERSE_OTPAIR_ONLY_MODE}" >&2
      exit 4
      ;;
  esac
fi
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_XVERSE_OTPAIR_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_XVERSE_OTPAIR_RERUN=1 to relaunch" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before xverse OT pair-mode launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
${PYTHON} ${ROOT}/ops/summarize_latentfm_single_background_candidate_decision_20260622.py --gate-json "\${eval_dir}/single_background_candidate_gate.json" --label ${run_name} --title "LatentFM xverse OT pair-mode candidate decision" --out-md "\${eval_dir}/SINGLE_BACKGROUND_CANDIDATE_DECISION.md"
EOF
...
\`\`\`bash
LATENTFM_XVERSE_OTPAIR_ACK=ot_pair_mode_codegate_pass bash ${ROOT}/ops/launch_latentfm_xverse_ot_pairmode_smokes_20260624.sh
\`\`\`
...
- Canonical split is used only after checkpoint freeze for no-harm diagnostics:
  \`${CANONICAL_SPLIT}\`
- Config: ot_pair_mode=${mode}, finetune_trainable_scope=all,
  total_steps=2000, ds_alpha=0.7, ds_loss_alpha=0.0.
...
# Run Status: latentfm_xverse_ot_pairmode_smokes_20260624
