...
DATA_DIR=${DATA_DIR:-${ROOT}/dataset/latentfm_full/xverse}
BIFLOW_DIR=${BIFLOW_DIR:-${ROOT}/dataset/biFlow_data}
SPLIT_FILE=${SPLIT_FILE:-${ROOT}/dataset/biFlow_data/split_seed42.json}
RUN_DIR=${RUN_DIR:?Set RUN_DIR to a LatentFM checkpoint directory containing best.pt}
RUN_LABEL=${RUN_LABEL:-$(basename "${RUN_DIR}")}
REPORT_LABEL=${REPORT_LABEL:-${RUN_LABEL}}
REPORT_TITLE=${REPORT_TITLE:-LatentFM xverse ${RUN_LABEL} Condition-Uncapped}
OUT_DIR=${OUT_DIR:-${RUN_DIR}/posthoc_eval_uncapped_20260621}
RUN_ROOT=${RUN_ROOT:-${ROOT}/runs/latentfm_xverse_single_uncapped_${REPORT_LABEL}_20260621}
LOG_ROOT=${LOG_ROOT:-${ROOT}/logs/latentfm_xverse_single_uncapped_${REPORT_LABEL}_20260621}
...
SESSION=${SESSION:-latentfm_xverse_single_uncapped_${REPORT_LABEL}_20260621}
SPLIT_JSON=${OUT_DIR}/split_group_eval_best_ode20_condition_uncapped_mse2048_mmd2048.json
FAMILY_JSON=${OUT_DIR}/condition_family_eval_best_ode20_condition_uncapped_mse2048_mmd2048.json
SPLIT_CI_JSON=${SPLIT_CI_JSON:-${ROOT}/reports/${REPORT_LABEL}_condition_uncapped_split_ci_20260621.json}
SPLIT_CI_MD=${SPLIT_CI_MD:-${ROOT}/reports/${REPORT_LABEL}_CONDITION_UNCAPPED_SPLIT_CI_20260621.md}
FAMILY_CI_JSON=${FAMILY_CI_JSON:-${ROOT}/reports/${REPORT_LABEL}_condition_uncapped_family_ci_20260621.json}
FAMILY_CI_MD=${FAMILY_CI_MD:-${ROOT}/reports/${REPORT_LABEL}_CONDITION_UNCAPPED_FAMILY_CI_20260621.md}
...
echo "[$(date '+%F %T %Z')] exact GPU status before xverse single uncapped posthoc" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
run_script="${RUN_ROOT}/run_xverse_single_uncapped_posthoc.sh"
cat > "${run_script}" <<EOF
#!/usr/bin/env bash
set -euo pipefail
...
${PYTHON} ${BOOTSTRAP} \\
  --eval-json ${SPLIT_JSON} \\
  --groups test test_multi_unseen2 \\
  --n-boot 2000 \\
  --seed 42 \\
  --title "${REPORT_TITLE} Split CI" \\
  --out-json ${SPLIT_CI_JSON} \\
  --out-md ${SPLIT_CI_MD}

${PYTHON} ${BOOTSTRAP} \\
  --eval-json ${FAMILY_JSON} \\
  --groups test_all family_gene family_drug structure_multi \\
  --n-boot 2000 \\
  --seed 42 \\
  --title "${REPORT_TITLE} Family CI" \\
  --out-json ${FAMILY_CI_JSON} \\
  --out-md ${FAMILY_CI_MD}
EOF
chmod +x "${run_script}"
...
tmux new -d -s "${SESSION}" \
  "bash -lc 'bash ${run_script} > ${LOG_ROOT}/xverse_single_uncapped_posthoc.log 2>&1; rc=\$?; echo \$rc > ${RUN_ROOT}/EXIT_CODE; date \"+%F %T %Z\" > ${RUN_ROOT}/FINISHED; exit \$rc'"
...
\`\`\`bash
RUN_DIR=${RUN_DIR} RUN_LABEL=${RUN_LABEL} REPORT_LABEL=${REPORT_LABEL} bash ${ROOT}/ops/launch_latentfm_xverse_single_uncapped_posthoc_20260621.sh
\`\`\`
...
${LOG_ROOT}/xverse_single_uncapped_posthoc.log
...
tail -n 50 ${LOG_ROOT}/xverse_single_uncapped_posthoc.log
nvidia-smi
\`\`\`
...
Condition-uncapped xverse single-checkpoint posthoc. Eval condition caps are
zero and MSE/MMD cell caps are 2048, matching the xverse 8k stage protocol.
EOF
