...
OUT_MD = ROOT / "reports/LATENTFM_PRIOR_CORRECTION_EVAL_20260619.md"
OUT_CSV = ROOT / "reports/latentfm_prior_correction_eval_20260619.csv"
OUT_JSON = ROOT / "reports/latentfm_prior_correction_eval_20260619.json"
...
def aggregate(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for label, ds, group, alpha, k in keys:
        ...
        if any("mmd" in v for v in vals):
            row.update(
                {
                    "mmd": avg("mmd"),
                    "mmd_biased": avg("mmd_biased"),
                    "mmd_clamped": avg("mmd_clamped"),
                }
            )
        out.append(row)
    return out
...
def evaluate(args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    with ctx:
        for ds_name in dataset.ds_names:
            ...
            for cond in dataset.ds_conds[ds_name]:
                ...
                pred_cells = torch.cat(pred_parts, dim=0).numpy().astype(np.float32)
                model_mean = pred_cells.mean(axis=0).astype(np.float32)
                gt_mmd_dev = None
                sigmas = None
                Dyy = None
                if bool(getattr(args, "compute_shifted_mmd", False)):
                    gt_mmd_dev = torch.from_numpy(gt_eval_np).to(device=device, dtype=torch.float32)
                    sigmas, Dyy = median_sigmas(gt_mmd_dev, return_D2=True)
                ...
                for k in k_values:
                    ...
                    for alpha in alpha_values:
                        ...
                        mmd_val = None
                        mmd_biased_val = None
                        mmd_clamped_val = None
                        if gt_mmd_dev is not None and sigmas is not None:
                            shift = (corrected - model_mean).astype(np.float32)
                            corrected_cells = torch.from_numpy(pred_cells + shift[None, :]).to(
                                device=device,
                                dtype=torch.float32,
                            )
                            mmd_val = float(mmd2_unbiased(corrected_cells, gt_mmd_dev, sigmas, Dyy=Dyy).item())
                            mmd_biased_val = float(mmd2_biased(corrected_cells, gt_mmd_dev, sigmas, Dyy=Dyy).item())
                            mmd_clamped_val = max(mmd_val, 0.0)
                            ...
                            rows.append(
                                {
                                    ...
                                    "mmd": mmd_val,
                                    "mmd_biased": mmd_biased_val,
                                    "mmd_clamped": mmd_clamped_val,
    ...
    meta = {
        ...
        "compute_shifted_mmd": bool(getattr(args, "compute_shifted_mmd", False)),
        "mmd_interpretation": (
            "If enabled, MMD is computed after translating all ODE-predicted cells "
            "by corrected_mean - model_mean. This is a conservative distribution "
            "gate for mean-level prior correction, not retrained model output."
        ),
        "device": str(device),
...
def write_outputs(rows: list[dict[str, Any]], summary: list[dict[str, Any]], meta: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    lines = [
        ...
        "## Summary",
        "",
        "| Dataset | Group | alpha | k | n | direct | pc | pp | MMD clamped | prior rate | missing comps |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    priority = [
        r
    ...
    for row in priority:
        lines.append(
            "| {dataset} | `{group}` | {alpha} | {k} | {n_conditions} | {direct} | {pc} | {pp} | {mmd_clamped} | {prior_available_rate} | {mean_missing_components} |".format(
                dataset=row["dataset"],
                group=row["group"],
                alpha=fmt(row["alpha"]),
                k=row["k"],
                ...
                pc=fmt(row["pc"]),
                pp=fmt(row["pp"]),
                mmd_clamped=fmt(row.get("mmd_clamped")),
    ...
    lines.extend(
        [
            ...
            "collapsing `pc`. If `--compute-shifted-mmd` is enabled, MMD is computed",
            "after translating the ODE-predicted distribution by the mean-level",
            "prior correction. This is a gate for distribution damage, not proof",
            "that a retrained model has internalized the correction.",
            "",
        ]
    )
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--compute-shifted-mmd", action="store_true")
