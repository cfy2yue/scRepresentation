...
RUN_ROOT=${PAIRWISE_GLOBAL_RUN_ROOT:-${ROOT}/runs/latentfm_pairwise_global_gene_mean_prior_20260621}
OUT_ROOT=${PAIRWISE_GLOBAL_OUT_ROOT:-${COUPLED}/output/latentfm_runs/pairwise_global_gene_mean_prior_20260621}
LOG_ROOT=${PAIRWISE_GLOBAL_LOG_ROOT:-${ROOT}/logs/latentfm_pairwise_global_gene_mean_prior_20260621}
...
PRIOR_WEIGHT=${PAIRWISE_GLOBAL_PRIOR_WEIGHT:-0.05}
ANCHOR_REPLAY_WEIGHT=${PAIRWISE_GLOBAL_ANCHOR_REPLAY_WEIGHT:-0.0}
ANCHOR_REPLAY_FILTER=${PAIRWISE_GLOBAL_ANCHOR_REPLAY_FILTER:-non_gene_multi}
RUN_NAME=${PAIRWISE_GLOBAL_RUN_NAME:-scf_pair_condadapter_global_genemean_prior005_4k}
SESSION=lfm_${RUN_NAME}
POSTHOC_SESSION=${PAIRWISE_GLOBAL_POSTHOC_SESSION:-latentfm_pair_global_genemean_posthoc_20260621}
SUMMARY_TAG=${PAIRWISE_GLOBAL_SUMMARY_TAG:-pairwise_global_gene_mean_prior}
SUMMARY_JSON=${ROOT}/reports/latentfm_${SUMMARY_TAG}_smoke_summary_20260621.json
SUMMARY_MD=${ROOT}/reports/LATENTFM_${SUMMARY_TAG^^}_SMOKE_SUMMARY_20260621.md
BOOTSTRAP_DIR=${ROOT}/reports/latentfm_${SUMMARY_TAG}_smoke_bootstrap_20260621
...
if [[ "${FORCE_PAIRWISE_GLOBAL_GENEMEAN_RERUN:-0}" != "1" && -e "${OUT_ROOT}/${RUN_NAME}" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_PAIRWISE_GLOBAL_GENEMEAN_RERUN=1 to relaunch" >&2
  exit 3
fi

echo "[$(date '+%F %T %Z')] exact GPU status before global-gene-mean prior launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
${PYTHON} -m model.latent.train \\
  ...
  --condition-prior-delta-loss-weight ${PRIOR_WEIGHT} \\
  ...
  --condition-prior-bank-split-file ${CANONICAL_SPLIT} \\
  --condition-prior-bank-aggregation gene_mean \\
  --anchor-replay-loss-weight ${ANCHOR_REPLAY_WEIGHT} \\
  --anchor-replay-loss-warmup-start 0 \\
  --anchor-replay-loss-warmup-end 1000 \\
  --anchor-replay-condition-filter ${ANCHOR_REPLAY_FILTER} \\
...
tmux new -d -s "${POSTHOC_SESSION}" \
  "bash -lc 'LATENTFM_PAIRWISE_RUN_ROOT=${RUN_ROOT} LATENTFM_PAIRWISE_OUT_ROOT=${OUT_ROOT} LATENTFM_PAIRWISE_RUN_NAME=${RUN_NAME} LATENTFM_PAIRWISE_SUMMARY_JSON=${SUMMARY_JSON} LATENTFM_PAIRWISE_SUMMARY_MD=${SUMMARY_MD} LATENTFM_PAIRWISE_BOOTSTRAP_DIR=${BOOTSTRAP_DIR} LATENTFM_PAIRWISE_POSTHOC_TITLE=latentfm_${SUMMARY_TAG}_posthoc_20260621 bash ${POSTHOC_SCRIPT}'"

cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# Run Status: latentfm_pairwise_global_gene_mean_prior_20260621

## Command

\`\`\`bash
PAIRWISE_GLOBAL_PRIOR_WEIGHT=${PRIOR_WEIGHT} PAIRWISE_GLOBAL_ANCHOR_REPLAY_WEIGHT=${ANCHOR_REPLAY_WEIGHT} PAIRWISE_GLOBAL_RUN_NAME=${RUN_NAME} bash ${ROOT}/ops/launch_latentfm_pairwise_global_gene_mean_prior_smoke_20260621.sh
\`\`\`
...
* \`${OUT_ROOT}/${RUN_NAME}/condition_prior_bank_summary.json\`
* \`${SUMMARY_MD}\`
* \`${BOOTSTRAP_DIR}/bootstrap_index.json\`
...
This is a guarded synthetic-combo smoke: global train-single \`gene_mean\` prior
targets are sampled from the canonical train bank, while the base flow and
condition-delta head stay frozen through \`pairwise_condition_adapter\`.
It differs from the simultaneously running same-dataset condition-prior adapter
by using \`condition_prior_bank_scope=global\` and
\`condition_prior_bank_aggregation=gene_mean\`, motivated by the CPU coverage
audit showing 238/244 test multi conditions globally covered versus 81/244
same-dataset covered.

Anchor replay is controlled by \`PAIRWISE_GLOBAL_ANCHOR_REPLAY_WEIGHT\`
(current value: \`${ANCHOR_REPLAY_WEIGHT}\`) and defaults to off. When enabled,
it uses metadata-only filter \`${ANCHOR_REPLAY_FILTER}\` and the frozen anchor
