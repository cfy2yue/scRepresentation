...
RUN_ROOT=${ROOT}/runs/latentfm_pairwise_adapter_only_20260621
OUT_ROOT=${COUPLED}/output/latentfm_runs/pairwise_adapter_only_20260621
LOG_ROOT=${ROOT}/logs/latentfm_pairwise_adapter_only_20260621
...
RUN_NAME=scf_pair_hadamard_adapteronly_prior010_e2_4k
SESSION=lfm_${RUN_NAME}
POSTHOC_SESSION=latentfm_pairwise_adapter_only_posthoc_20260621
...
if [[ "${FORCE_PAIRWISE_ADAPTER_RERUN:-0}" != "1" && -e "${OUT_ROOT}/${RUN_NAME}" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_PAIRWISE_ADAPTER_RERUN=1 to relaunch" >&2
  exit 3
fi

echo "[$(date '+%F %T %Z')] exact GPU status before pairwise-adapter launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
${PYTHON} -m model.latent.train \\
  ...
  --finetune-trainable-scope pairwise_adapter \\
  --batch-size 64 \\
  --grad-accum-steps 1 \\
  --min-cells 32 \\
  --scale-noise 0.02 \\
...
tmux new -d -s "${POSTHOC_SESSION}" \
  "bash -lc 'LATENTFM_PAIRWISE_RUN_ROOT=${RUN_ROOT} LATENTFM_PAIRWISE_OUT_ROOT=${OUT_ROOT} LATENTFM_PAIRWISE_RUN_NAME=${RUN_NAME} LATENTFM_PAIRWISE_SUMMARY_JSON=${ROOT}/reports/latentfm_pairwise_adapter_only_smoke_summary_20260621.json LATENTFM_PAIRWISE_SUMMARY_MD=${ROOT}/reports/LATENTFM_PAIRWISE_ADAPTER_ONLY_SMOKE_SUMMARY_20260621.md LATENTFM_PAIRWISE_BOOTSTRAP_DIR=${ROOT}/reports/latentfm_pairwise_adapter_only_smoke_bootstrap_20260621 LATENTFM_PAIRWISE_POSTHOC_TITLE=latentfm_pairwise_adapter_only_posthoc_20260621 bash ${POSTHOC_SCRIPT}'"

cat > "${RUN_ROOT}/RUN_STATUS.md" <<EOF
# Run Status: latentfm_pairwise_adapter_only_20260621

## Command

\`\`\`bash
bash ${ROOT}/ops/launch_latentfm_pairwise_adapter_only_smoke_20260621.sh
\`\`\`
...
* \`${ROOT}/reports/LATENTFM_PAIRWISE_ADAPTER_ONLY_SMOKE_SUMMARY_20260621.md\`
* \`${ROOT}/reports/latentfm_pairwise_adapter_only_smoke_bootstrap_20260621/bootstrap_index.json\`

## Current status

Started training and posthoc watcher. This run freezes the warm-started model
except \`pert_encoder.pair_to_out.*\` via
\`--finetune-trainable-scope pairwise_adapter\`.

## Gate

Anchor-relative gate: preserve the pairwise full-finetune multi-unseen pp
signal while eliminating single/drug harm and deterministic aggregate MMD harm.
Matched canonical-refinetune comparison remains required before promotion.
EOF
