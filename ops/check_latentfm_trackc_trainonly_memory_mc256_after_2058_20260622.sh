...
SUMMARY_MD=${ROOT}/reports/LATENTFM_TRACKC_TRAINONLY_MEMORY_MC256_DECISION_SUMMARY_20260622.md
SUMMARY_CSV=${ROOT}/reports/latentfm_trackc_trainonly_memory_mc256_decision_summary_20260622.csv
...
now_epoch=$(date +%s)
boundary_epoch=$(date -d '2026-06-22 20:58:00' +%s)
if (( now_epoch < boundary_epoch )); then
  echo "Refusing to check before 2026-06-22 20:58:00 CST" >&2
  exit 3
fi

echo "# Track C train-only memory mc256 one-shot status"
echo "checked_at=$(date '+%F %T %Z')"
echo
echo "## tmux"
tmux ls 2>/dev/null | grep -E 'trackc_route_(train|posthoc)_xverse_trackc_mem256' || echo "no mc256 memory tmux sessions"
echo
