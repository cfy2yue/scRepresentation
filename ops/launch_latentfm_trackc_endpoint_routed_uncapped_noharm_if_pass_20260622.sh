...
OUT_DIR=${COUPLED}/output/latentfm_runs/xverse_trackc_endpoint_routed_20260622/${RUN_NAME}
...
MANIFEST=${ROOT}/reports/latentfm_trackc_endpoint_routed_uncapped_noharm_manifest_20260622.json
LABEL=latentfm_trackc_endpoint_routed_uncapped_noharm_20260622
...
if [[ "${status}" != "trackc_smoke_support_pass_needs_uncapped_noharm_before_query" ]]; then
  echo "Refusing endpoint-routed uncapped no-harm: decision status is '${status}', not pass." >&2
  exit 5
fi
...
payload = {
    "purpose": "Track C endpoint-routed canonical no-harm only; no held-out query",
