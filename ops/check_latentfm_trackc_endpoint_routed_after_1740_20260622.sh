...
EARLIEST_EPOCH=$(date -d '2026-06-22 17:40:00 CST' +%s)
NOW_EPOCH=$(date +%s)

if (( NOW_EPOCH < EARLIEST_EPOCH )); then
  echo "Refusing endpoint-routed posthoc check before 2026-06-22 17:40:00 CST" >&2
  exit 3
fi
...
echo "[$(date '+%F %T %Z')] endpoint-routed posthoc/decision lightweight status check"
