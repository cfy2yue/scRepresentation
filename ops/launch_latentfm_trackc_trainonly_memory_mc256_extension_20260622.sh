...
OUT_ROOT=${ROOT}/CoupledFM/output/latentfm_runs/xverse_trackc_trainonly_memory_parallel_mc256_ext_20260622
LOG_ROOT=${ROOT}/logs/latentfm_xverse_trackc_trainonly_memory_parallel_mc256_ext_20260622
...
for run in runs:
    ...
    if not path.is_file():
        raise SystemExit(f"latest decision missing, do not launch memory extension: {path}")
    ...
    if status == "trackc_smoke_support_pass_needs_uncapped_noharm_before_query":
        raise SystemExit(f"latest checkpoint passed for {run}; do not launch memory extension")
    if not status.startswith("trackc_smoke_fail_"):
        raise SystemExit(f"latest checkpoint status not closed for {run}: {status}")
print("latest_checkpoint_closed_for_memory_extension")
PY
...
row = {
    ...
    "condition_prior_bank_max_cells": 256,
    "hypothesis": sys.argv[11],
    "run_status": f"/data/cyx/1030/scLatent/runs/latentfm_xverse_trackc_trainonly_memory_parallel_mc256_ext_20260622/{run_name}/RUN_STATUS.md",
...
launch_one xverse_trackc_mem256ext_cp_ep025_replay2_all_2k_seed42 1 \
  condition_prior_adapter off 0.25 0 500 2.0 all \
  "Lower condition-prior memory endpoint dose tests whether support transfer can emerge with less canonical harm risk."

launch_one xverse_trackc_mem256ext_pc_ep025_replay2_all_2k_seed42 1 \
  pairwise_condition_adapter hadamard_mean 0.25 0 500 2.0 all \
  "Lower pairwise memory endpoint dose tests whether interaction capacity helps without overfitting the memory teacher."

launch_one xverse_trackc_mem256ext_cp_ep050_replay4_nongm_2k_seed42 1 \
  condition_prior_adapter off 0.50 0 500 4.0 non_gene_multi \
  "Condition-prior memory endpoint with stronger non-gene-multi replay tests canonical protection under the mid-dose rule."

launch_one xverse_trackc_mem256ext_cp_ep075_replay4_nongm_2k_seed42 2 \
  condition_prior_adapter off 0.75 0 500 4.0 non_gene_multi \
  "Intermediate condition-prior dose plus stronger canonical-only replay tests dose response between w0.5 and w1.0."

launch_one xverse_trackc_mem256ext_pc_ep100_replay2_all_2k_seed42 2 \
  pairwise_condition_adapter hadamard_mean 1.00 0 500 2.0 all \
  "Stronger pairwise memory endpoint tests whether the train-only memory signal needs higher interaction-path pressure."

launch_one xverse_trackc_mem256ext_cp_ep050del_replay4_all_2k_seed42 3 \
  condition_prior_adapter off 0.50 500 1000 4.0 all \
  "Delayed condition-prior memory endpoint tests whether preserving the anchor early reduces canonical harm."

launch_one xverse_trackc_mem256ext_pc_ep050del_replay2_all_2k_seed42 3 \
  pairwise_condition_adapter hadamard_mean 0.50 500 1000 2.0 all \
  "Delayed pairwise memory endpoint tests whether interaction capacity benefits from later teacher pressure."

launch_one xverse_trackc_mem256ext_pc_ep075_replay4_all_2k_seed42 3 \
  pairwise_condition_adapter hadamard_mean 0.75 0 500 4.0 all \
  "Intermediate pairwise dose with stronger all-condition replay tests support gain under tighter no-harm regularization."

echo "trainonly_memory_mc256_extension_launched"
