#!/usr/bin/env python3
"""Summarize xverse train-strategy smoke decisions after posthoc completes."""
...
RUN_ROOT = ROOT / "runs/latentfm_xverse_train_strategy_smokes_20260624"
OUT_JSON = ROOT / "reports/latentfm_xverse_train_strategy_smokes_decision_20260624.json"
OUT_MD = ROOT / "reports/LATENTFM_XVERSE_TRAIN_STRATEGY_SMOKES_DECISION_20260624.md"
...
def decide(rows: list[dict]) -> dict:
    ...
    if pending:
        ...
        action = "close_balanced_exposure_loss_branch_and_move_to_type_adapter_or_scaling_split_builder"
    return {
        "status": status,
        "action": action,
...
def main() -> None:
    ...
    OUT_MD.write_text(
        f"""# LatentFM xverse Train-Strategy Smokes Decision
...
- Summarizes predeclared xverse train-strategy smokes only.
