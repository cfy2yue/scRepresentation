...
RUN_NAME=xverse_trackc_residual_operator_memall_resid_ep050_replay2_2k_seed42
...
WINDOW="2026-06-23 02:10:00"

now_epoch=$(date +%s)
window_epoch=$(date -d "${WINDOW}" +%s)
if (( now_epoch < window_epoch )); then
  echo "Refusing to check before ${WINDOW} CST; residual-operator GPU smoke is a long task." >&2
  exit 3
fi

echo "[$(date '+%F %T %Z')] Track C residual-operator guarded check"
...
if [[ -f "${RUN_ROOT}/posthoc_eval/support_anchor_split_ode20.json" && -f "${RUN_ROOT}/posthoc_eval/support_candidate_split_ode20.json" ]]; then
  python "${ROOT}/ops/evaluate_latentfm_trackc_residual_operator_route_gap_gate_20260623.py" \
    --run-name "${RUN_NAME}" \
