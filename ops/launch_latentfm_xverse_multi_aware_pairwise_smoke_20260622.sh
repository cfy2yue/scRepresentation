...
RUN_NAME=${LATENTFM_XVERSE_MULTI_AWARE_RUN_NAME:-xverse_multiaware_pairwise_adapter_wn_2k_r2}
RUN_ROOT=${LATENTFM_XVERSE_MULTI_AWARE_RUN_ROOT:-${ROOT}/runs/latentfm_xverse_multi_aware_pairwise_smoke_20260622}
OUT_ROOT=${LATENTFM_XVERSE_MULTI_AWARE_OUT_ROOT:-${COUPLED}/output/latentfm_runs/xverse_multi_aware_pairwise_smoke_20260622}
LOG_ROOT=${LATENTFM_XVERSE_MULTI_AWARE_LOG_ROOT:-${ROOT}/logs/latentfm_xverse_multi_aware_pairwise_smoke_20260622}
DATA_DIR=${ROOT}/dataset/latentfm_full/xverse
BIFLOW_DIR=${ROOT}/dataset/biFlow_data
FOCUS_SPLIT=${ROOT}/dataset/biFlow_data/split_seed42_multi_aware_v1_finetune_wn.json
FULL_MULTI_SPLIT=${ROOT}/dataset/biFlow_data/split_seed42_multi_aware_v1.json
...
session=${LATENTFM_XVERSE_MULTI_AWARE_SESSION:-latentfm_xverse_multiaware_pairwise_20260622_r2}
...
if [[ -e "${out_dir}" && "${FORCE_XVERSE_MULTI_AWARE_RERUN:-0}" != "1" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_XVERSE_MULTI_AWARE_RERUN=1 to relaunch" >&2
  exit 3
fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before xverse multi-aware pairwise launch" | tee "${run_root}/logs/gpu_launch_audit.log"
...
if [[ -n "${LATENTFM_XVERSE_MULTI_AWARE_GPU:-}" ]]; then
  gpu_json="${LATENTFM_XVERSE_MULTI_AWARE_GPU_AUDIT_JSON:-manual_prelaunch_audit}"
  "${PYTHON}" - "${LATENTFM_XVERSE_MULTI_AWARE_GPU}" "${gpu_json}" "${assignment_json}" <<'PY'
import json
import sys
...
export FINETUNE_TRAINABLE_SCOPE=pairwise_adapter
export PERT_PAIRWISE_MODE=hadamard_mean
...
export PYTHONPATH=${COUPLED}:\${PYTHONPATH:-}
export PERT_EMBED_SOURCE=scgpt_embed_gene
eval_dir=${run_root}/posthoc_eval_multiaware
mkdir -p "\${eval_dir}"
common=(--data-dir ${DATA_DIR} --biflow-dir ${BIFLOW_DIR} --split-file ${FULL_MULTI_SPLIT} --gpu 0 --ode-steps 20 --max-chunk 512 --eval-max-conditions 0 --eval-max-conditions-per-dataset 0 --eval-max-mse-cells 1024 --eval-max-mmd-cells 1024)
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${ANCHOR_CKPT} --groups test test_single test_multi test_multi_seen_combo test_multi_unseen1_or_more_same_background test_multi_cross_background_seen test_multi_hard_unseen --out "\${eval_dir}/split_group_eval_anchor_ode20_multiaware.json" "\${common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${ANCHOR_CKPT} --groups test_all family_gene family_drug structure_single structure_multi test_single test_multi --out "\${eval_dir}/condition_family_eval_anchor_ode20_multiaware.json" "\${common[@]}"
${PYTHON} -m model.latent.eval_split_groups --checkpoint ${out_dir}/best.pt --groups test test_single test_multi test_multi_seen_combo test_multi_unseen1_or_more_same_background test_multi_cross_background_seen test_multi_hard_unseen --out "\${eval_dir}/split_group_eval_candidate_ode20_multiaware.json" "\${common[@]}"
${PYTHON} -m model.latent.eval_condition_families --checkpoint ${out_dir}/best.pt --groups test_all family_gene family_drug structure_single structure_multi test_single test_multi --out "\${eval_dir}/condition_family_eval_candidate_ode20_multiaware.json" "\${common[@]}"
EOF
...
\`\`\`bash
bash ${ROOT}/ops/launch_latentfm_xverse_multi_aware_pairwise_smoke_20260622.sh
\`\`\`
...
* \`${run_root}/posthoc_eval_multiaware/split_group_eval_candidate_ode20_multiaware.json\`
* \`${run_root}/posthoc_eval_multiaware/condition_family_eval_candidate_ode20_multiaware.json\`
...
This is a separate multi-aware Wessels/Norman fine-tune branch, not a canonical zero-shot result.
Training split: \`${FOCUS_SPLIT}\`.
Evaluation split: \`${FULL_MULTI_SPLIT}\`.
Warm-start anchor: \`${ANCHOR_CKPT}\`.
Trainable scope: \`pairwise_adapter\`; pairwise mode: \`hadamard_mean\`.
EOF
