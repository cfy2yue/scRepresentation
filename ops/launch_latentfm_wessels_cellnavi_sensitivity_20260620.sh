...
OUT_ROOT=${COUPLED}/output/latentfm_runs/wessels_condition_source_sensitivity_20260620
LOG_ROOT=${ROOT}/logs/latentfm_wessels_cellnavi_sensitivity_20260620
...
POSTHOC_SCRIPT=${ROOT}/ops/run_latentfm_wessels_cellnavi_posthoc_20260620.sh
...
GENE_CACHE=${ROOT}/pretrainckpt/genepert_cache/cellnavi_embed_gene
RUN_NAME=scf_prior010_upperbound_wessels_cellnavi_4k
POSTHOC_SESSION=latentfm_wessels_cellnavi_posthoc_20260620

mkdir -p "${RUN_ROOT}/logs" "${OUT_ROOT}" "${LOG_ROOT}" "${ROOT}/reports"
for required in "${DATA_DIR}" "${SPLIT_FILE}" "${BASE_INIT}" "${GENE_CACHE}/manifest.json" "${DECISION_JSON}"; do
  if [[ ! -e "${required}" ]]; then
...
print(json.loads(Path(sys.argv[1]).read_text(encoding="utf-8")).get("next_action", ""))
PY
)"
case "${next_action}" in
  run_dataset_upper_bound_diagnostics|run_dataset_upper_bound_before_all_split_balance|prioritize_combo_aware_condition_modeling_or_latent_sensitivity)
    ;;
  *)
    echo "Focus decision does not authorize Wessels condition-source sensitivity: ${next_action}" >&2
    exit 3
    ;;
esac

if [[ -e "${OUT_ROOT}/${RUN_NAME}" && "${FORCE_WESSELS_CELLNAVI_RERUN:-0}" != "1" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_WESSELS_CELLNAVI_RERUN=1 to override" >&2
  exit 6
fi

echo "[$(date '+%F %T %Z')] exact GPU status before Wessels CellNavi launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
"${PYTHON}" "${GPU_HELPER}" --samples 3 --interval-seconds 10 --need 1 --max-jobs-per-gpu 3 --json-only \
...
${PYTHON} -m model.latent.train \\
  ...
  --pert-condition-embedding-source cellnavi_embed_gene \\
...
# Run Status: latentfm_wessels_cellnavi_sensitivity_20260620

## Command

\`\`\`bash
bash ${ROOT}/ops/launch_latentfm_wessels_cellnavi_sensitivity_20260620.sh
\`\`\`
...
cat ${ROOT}/runs/latentfm_wessels_cellnavi_posthoc_20260620/EXIT_CODE 2>/dev/null || echo posthoc: still-running
tmux ls
nvidia-smi
\`\`\`
...
Wessels-only CellNavi condition-source sensitivity. Shape-mismatched init
weights are skipped by CoupledFM commit a063cfb.
...
tmux ls | grep -E "${session}|${POSTHOC_SESSION}" || true
