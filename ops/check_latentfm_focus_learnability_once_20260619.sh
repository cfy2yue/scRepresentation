...
POSTHOC_ROOT=${ROOT}/runs/latentfm_focus_learnability_posthoc_20260619
NOT_BEFORE=${NOT_BEFORE:-"2026-06-19 23:51:08"}
TZ_NAME=${TZ_NAME:-"Asia/Shanghai"}
REPORT=${REPORT:-"${ROOT}/reports/LATENTFM_FOCUS_LEARNABILITY_ONE_SHOT_STATUS_$(date +%Y%m%d_%H%M%S).txt"}
...
echo "[$(date '+%F %T %Z')] one-shot LatentFM focus-learnability status check"
echo "report=${REPORT}"
echo
...
echo "tmux sessions:"
tmux ls | grep -E 'lfm_scf_prior010_inject_nwg_focus|latentfm_focus_learnability_posthoc_20260619' || true
echo

for run in \
  scf_prior010_inject_nwg_focus_4k \
  scf_prior010_inject_nwg_focus_dsloss05_4k
do
  echo "### ${run}"
...
echo "Expected focus summary artifacts:"
for p in \
  "${ROOT}/reports/latentfm_focus_learnability_stablecaps_summary_20260619.csv" \
  "${ROOT}/reports/latentfm_focus_learnability_stablecaps_summary_20260619_gate.json" \
