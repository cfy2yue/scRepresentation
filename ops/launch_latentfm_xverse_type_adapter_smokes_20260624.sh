...
if [[ "${LATENTFM_XVERSE_TYPEADAPTER_ACK:-}" != "type_adapter_scope_pycompile_pass" ]]; then
  cat >&2 <<'EOF'
Refusing to launch xverse type-adapter smokes.

Set:
  LATENTFM_XVERSE_TYPEADAPTER_ACK=type_adapter_scope_pycompile_pass
...
RUN_ROOT=${LATENTFM_XVERSE_TYPEADAPTER_RUN_ROOT:-${ROOT}/runs/latentfm_xverse_type_adapter_smokes_20260624}
OUT_ROOT=${LATENTFM_XVERSE_TYPEADAPTER_OUT_ROOT:-${COUPLED}/output/latentfm_runs/xverse_type_adapter_smokes_20260624}
LOG_ROOT=${LATENTFM_XVERSE_TYPEADAPTER_LOG_ROOT:-${ROOT}/logs/latentfm_xverse_type_adapter_smokes_20260624}
...
declare -a HYPOTHESES=(
  "Per-dimension perturbation-type scaling can correct cross-background response amplitude while keeping the warm-started xverse anchor frozen."
  "A small per-dimension gated type scale can add nonlinear type calibration without moving the warm-started xverse anchor body."
)
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_XVERSE_TYPEADAPTER_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_XVERSE_TYPEADAPTER_RERUN=1 to relaunch" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before xverse type-adapter launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
export FINETUNE_TRAINABLE_SCOPE=type_adapter
export PERT_TYPE_ADAPTER_MODE=${mode}
...
${PYTHON} ${ROOT}/ops/summarize_latentfm_single_background_candidate_decision_20260622.py --gate-json "\${eval_dir}/single_background_candidate_gate.json" --label ${run_name} --title "LatentFM xverse type-adapter candidate decision" --out-md "\${eval_dir}/SINGLE_BACKGROUND_CANDIDATE_DECISION.md"
EOF
...
\`\`\`bash
LATENTFM_XVERSE_TYPEADAPTER_ACK=type_adapter_scope_pycompile_pass bash ${ROOT}/ops/launch_latentfm_xverse_type_adapter_smokes_20260624.sh
\`\`\`
...
- Canonical split is used only after checkpoint freeze for no-harm diagnostics:
  \`${CANONICAL_SPLIT}\`
- Config: finetune_trainable_scope=type_adapter, pert_type_adapter_mode=${mode},
  lr=3e-4, total_steps=3000, ds_alpha=0.7, ds_loss_alpha=0.0.
...
# Run Status: latentfm_xverse_type_adapter_smokes_20260624
