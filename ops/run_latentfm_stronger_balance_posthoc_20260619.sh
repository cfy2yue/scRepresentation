...
TRAIN_RUN_ROOT=${ROOT}/runs/latentfm_stronger_balance_after_focus_20260619
RUN_ROOT=${ROOT}/runs/latentfm_stronger_balance_posthoc_20260619
TRAIN_OUT_ROOT=${COUPLED}/output/latentfm_runs/stronger_balance_after_focus_20260619
BASELINE_DIR=${COUPLED}/output/latentfm_runs/condition_prior_teacher_injection_20260619/scf_prior010_inject_e2_4k
BASELINE_STAGE_DIR=${RUN_ROOT}/baseline_scf_prior010_inject_e2_4k
...
# Run Status: latentfm_stronger_balance_posthoc_20260619

## Command

\`\`\`bash
bash ${ROOT}/ops/run_latentfm_stronger_balance_posthoc_20260619.sh
\`\`\`
...
tmux: \`latentfm_stronger_balance_posthoc_20260619\`
...
Waiting for stronger-balance training EXIT_CODE files.
...
{
  echo "[$(date '+%F %T %Z')] wait for stronger-balance training exits"
  while true; do
    all_done=1
    ...
    echo "[$(date '+%F %T %Z')] training still running; sleep 1800"
    sleep 1800
  done

  for run in "${RUN_A}" "${RUN_B}"; do
    rc="$(cat "${TRAIN_RUN_ROOT}/${run}.EXIT_CODE")"
    if [[ "${rc}" != "0" ]]; then
      echo "Training failed for ${run}: EXIT_CODE=${rc}" >&2
      exit 10
    fi
  done

  echo "[$(date '+%F %T %Z')] exact GPU status before stronger-balance posthoc"
...
PY
  ...
  run_posthoc() {
    ...
    local output_dir="$3"
    local posthoc="${output_dir}/posthoc_eval"
    mkdir -p "${posthoc}"
    echo "[$(date '+%F %T %Z')] posthoc ${run}"
    test -f "${checkpoint_dir}/best.pt"
  ...
  run_posthoc "baseline_scf_prior010_inject_e2_4k" "${BASELINE_DIR}" "${BASELINE_STAGE_DIR}"
  run_posthoc "${RUN_A}" "${TRAIN_OUT_ROOT}/${RUN_A}" "${TRAIN_OUT_ROOT}/${RUN_A}"
  run_posthoc "${RUN_B}" "${TRAIN_OUT_ROOT}/${RUN_B}" "${TRAIN_OUT_ROOT}/${RUN_B}"

  "${PYTHON}" "${ROOT}/ops/summarize_latentfm_sampling_smokes_stablecaps_20260619.py" \
    --baseline-name scf_prior010_inject_e2_4k \
    --baseline-dir "${BASELINE_STAGE_DIR}" \
    --run "${RUN_A}" "${TRAIN_OUT_ROOT}/${RUN_A}" \
    --run "${RUN_B}" "${TRAIN_OUT_ROOT}/${RUN_B}" \
    --out-csv "${ROOT}/reports/latentfm_stronger_balance_stablecaps_summary_20260619.csv" \
    --out-md "${ROOT}/reports/LATENTFM_STRONGER_BALANCE_STABLECAPS_SUMMARY_20260619.md"

  echo "[$(date '+%F %T %Z')] finished stronger-balance posthoc"
} > "${log}" 2>&1
