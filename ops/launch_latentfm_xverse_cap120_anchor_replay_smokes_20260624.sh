...
if [[ "${LATENTFM_XVERSE_CAP120_REPLAY_ACK:-}" != "cap120_internal_signal_noharm_mechanism" ]]; then
  cat >&2 <<'EOF'
Refusing to launch cap120 anchor-replay smokes.

Set:
  LATENTFM_XVERSE_CAP120_REPLAY_ACK=cap120_internal_signal_noharm_mechanism

Required prereads:
  reports/LATENTFM_XVERSE_SCALING_COUNT_SMOKES_DECISION_20260624.md
  reports/LATENTFM_XVERSE_DATASET_VETO_ROUTE_GATE_20260624.md
EOF
  exit 4
fi

RUN_ROOT=${ROOT}/runs/latentfm_xverse_cap120_anchor_replay_smokes_20260624
OUT_ROOT=${COUPLED}/output/latentfm_runs/xverse_cap120_anchor_replay_smokes_20260624
LOG_ROOT=${ROOT}/logs/latentfm_xverse_cap120_anchor_replay_smokes_20260624
...
SUMMARIZER=${ROOT}/ops/summarize_latentfm_xverse_cap120_anchor_replay_smokes_20260624.py
...
for required in \
  ...
  "${ROOT}/reports/LATENTFM_XVERSE_DATASET_VETO_ROUTE_GATE_20260624.md"; do
  if [[ ! -e "${required}" ]]; then
...
declare -a WEIGHTS=("0.5" "1.0")
declare -a HYPOTHESES=(
  "Cap120 retains the strongest internal cross-background signal; anchor replay w=0.5 tests whether mild frozen-anchor consistency reduces family/no-harm drift without erasing signal."
  "Cap120 retains the strongest internal cross-background signal; anchor replay w=1.0 tests whether stronger frozen-anchor consistency controls family MMD drift while keeping material cross-background signal."
)
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ -e "${out_dir}" && "${FORCE_LATENTFM_XVERSE_CAP120_REPLAY_RERUN:-0}" != "1" ]]; then
    echo "Output exists for ${run_name}; set FORCE_LATENTFM_XVERSE_CAP120_REPLAY_RERUN=1 to relaunch" >&2
    exit 3
  fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before cap120 anchor-replay launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
export ANCHOR_REPLAY_LOSS_WEIGHT=${weight}
...
\`\`\`bash
LATENTFM_XVERSE_CAP120_REPLAY_ACK=cap120_internal_signal_noharm_mechanism bash ${ROOT}/ops/launch_latentfm_xverse_cap120_anchor_replay_smokes_20260624.sh
\`\`\`
...
* \`${ROOT}/reports/LATENTFM_XVERSE_CAP120_ANCHOR_REPLAY_SMOKES_DECISION_20260624.md\`
...
- Split: \`${SPLIT_FILE}\`
- Anchor replay weight: \`${weight}\`, filter: \`all\`, checkpoint: \`${ANCHOR_CKPT}\`, use EMA: \`1\`.
