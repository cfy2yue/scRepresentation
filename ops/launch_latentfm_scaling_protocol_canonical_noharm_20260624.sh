...
if [[ "${LATENTFM_SCALING_PROTOCOL_CANONICAL_ACK:-}" != "matrix_internal_pass_frozen" ]]; then
  cat >&2 <<'EOF'
Refusing to launch scaling protocol canonical no-harm.

Set:
  LATENTFM_SCALING_PROTOCOL_CANONICAL_ACK=matrix_internal_pass_frozen
...
MATRIX_RUN_ROOT=${ROOT}/runs/latentfm_scaling_protocol_matrix_20260624
TRAIN_OUT_ROOT=${COUPLED}/output/latentfm_runs/scaling_protocol_matrix_20260624
LOG_ROOT=${ROOT}/logs/latentfm_scaling_protocol_canonical_noharm_20260624
...
mapfile -t RUN_NAMES < <("${PYTHON}" - "${MATRIX_JSON}" <<'PY'
import json
import sys
from pathlib import Path
obj = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
decision = obj.get("decision") or {}
if decision.get("status") != "protocol_matrix_internal_pass":
    raise SystemExit(f"matrix decision not passed: {decision.get('status')!r}")
passed = set(decision.get("passed") or [])
for row in obj.get("rows") or []:
    if row.get("arm") in passed:
        print(row["name"])
PY
)
...
need=${#RUN_NAMES[@]}
if (( need <= 0 )); then
  echo "No passed protocol matrix arms to evaluate." >&2
  exit 2
fi
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  if [[ ! -e "${MATRIX_RUN_ROOT}/${run_name}/POSTHOC_EXIT_CODE" || "$(cat "${MATRIX_RUN_ROOT}/${run_name}/POSTHOC_EXIT_CODE")" != "0" ]]; then
    echo "Matrix posthoc not complete for ${run_name}; refusing canonical launch." >&2
    exit 2
  fi
  run_dir=${RUN_ROOT}/${run_name}
  session=lfm_scaling_proto_canon_${run_name}
...
echo "[$(date '+%F %T %Z')] exact GPU status before scaling protocol canonical no-harm launch" | tee "${RUN_ROOT}/logs/gpu_launch_audit.log"
...
${PYTHON} ${ROOT}/ops/summarize_latentfm_single_background_candidate_decision_20260622.py --gate-json "\${eval_dir}/single_background_candidate_gate.json" --label ${run_name} --title "LatentFM scaling protocol canonical no-harm decision" --out-md "\${eval_dir}/SINGLE_BACKGROUND_CANDIDATE_DECISION.md"
${PYTHON} ${SUMMARIZER}
EOF
  chmod +x "${script}"
...
# Run Status: scaling protocol canonical no-harm ${run_name}

## Command

\`\`\`bash
LATENTFM_SCALING_PROTOCOL_CANONICAL_ACK=matrix_internal_pass_frozen bash ${ROOT}/ops/launch_latentfm_scaling_protocol_canonical_noharm_20260624.sh
\`\`\`
...
* \`${ROOT}/reports/LATENTFM_SCALING_PROTOCOL_CANONICAL_NOHARM_DECISION_20260624.md\`
...
# Run Status: latentfm_scaling_protocol_canonical_noharm_20260624
...
Long GPU posthoc evaluation batch.
...
$(printf '* `%s`\n' "${RUN_NAMES[@]/#/lfm_scaling_proto_canon_}")
...
Started canonical no-harm for ${need} passed protocol arm(s).
EOF
