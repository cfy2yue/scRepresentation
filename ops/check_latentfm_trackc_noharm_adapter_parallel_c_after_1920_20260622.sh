...
if (( NOW_EPOCH < EARLIEST_EPOCH )); then
  echo "Refusing no-harm adapter block C check before 2026-06-22 19:20:00 CST" >&2
  exit 3
fi

runs=(
  xverse_trackc_noharm_cp_ep050_replay2_all_2k_seed42
  xverse_trackc_noharm_cp_ep100_replay2_all_2k_seed42
  xverse_trackc_noharm_cp_ep050del_replay2_all_2k_seed42
  xverse_trackc_noharm_cp_ep100del_replay4_all_2k_seed42
  ...
  xverse_trackc_noharm_cp_ep050_replay4_nongm_2k_seed42
  xverse_trackc_noharm_cp_ep100del_replay4_nongm_2k_seed42
  xverse_trackc_noharm_pc_ep050_replay4_nongm_2k_seed42
  xverse_trackc_noharm_pc_ep100del_replay4_nongm_2k_seed42
)

echo "[$(date '+%F %T %Z')] no-harm adapter block C lightweight status check"
for run in "${runs[@]}"; do
