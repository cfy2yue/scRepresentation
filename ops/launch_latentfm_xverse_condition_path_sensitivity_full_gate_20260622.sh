...
if [[ "${LATENTFM_XVERSE_COND_PATH_FULL_ACK:-}" != "validate_failed_cap8_gate" ]]; then
  echo "Refusing to launch: set LATENTFM_XVERSE_COND_PATH_FULL_ACK=validate_failed_cap8_gate" >&2
  exit 2
fi

GPU="${LATENTFM_COND_PATH_FULL_GPU:-1}"
RUN_NAME="xverse_condition_path_sensitivity_v2_true_shuffle_zero_full_cell256_ode10"
...
SESSION="latentfm_xverse_condpath_sens_full_20260622"
...
\`\`\`bash
LATENTFM_XVERSE_COND_PATH_FULL_ACK=validate_failed_cap8_gate LATENTFM_COND_PATH_FULL_GPU=${GPU} bash /data/cyx/1030/scLatent/ops/launch_latentfm_xverse_condition_path_sensitivity_full_gate_20260622.sh
\`\`\`
...
\`/data/cyx/1030/scLatent/reports/gpu_selection_condition_path_sensitivity_full_20260622_0336.json\`
...
* \`${RUN_ROOT}/condition_path_sensitivity_full_gate.json\`
* \`${RUN_ROOT}/CONDITION_PATH_SENSITIVITY_FULL_GATE.md\`
...
Full internal-val validation of the failed cap8 P9 gate. Uses train-only v2
internal proxy groups only and all available proxy conditions
(\`max_conditions_per_dataset=0\`), max 256 cells per condition, ODE steps
10. It does not train and does not use canonical test for gating.
EOF
...
/data/cyx/software/miniconda3/envs/scdfm/bin/python '${SCRIPT}' \
  ...
  --max-cells 256 \
  --max-chunk 128 \
  --max-conditions-per-dataset 0 \
  --out-json '${RUN_ROOT}/condition_path_sensitivity_full_gate.json' \
  --out-md '${RUN_ROOT}/CONDITION_PATH_SENSITIVITY_FULL_GATE.md' \
