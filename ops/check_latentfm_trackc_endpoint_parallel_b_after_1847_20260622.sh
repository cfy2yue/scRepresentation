...
EARLIEST_EPOCH=$(date -d '2026-06-22 18:47:00 CST' +%s)
NOW_EPOCH=$(date +%s)

if (( NOW_EPOCH < EARLIEST_EPOCH )); then
  echo "Refusing endpoint parallel block B check before 2026-06-22 18:47:00 CST" >&2
  exit 3
fi
...
echo "[$(date '+%F %T %Z')] endpoint parallel block B lightweight status check"
for run in "${runs[@]}"; do
