...
if [[ "${LATENTFM_XVERSE_SAMPLING_ACK:-}" != "cpu_gate_passed_single_candidate" ]]; then
  cat >&2 <<'EOF'
Refusing to launch conservative sampling smoke.

This is a gated P3 follow-up. First pass the CPU sampling gate, then relaunch
with:

  LATENTFM_XVERSE_SAMPLING_ACK=cpu_gate_passed_single_candidate

Do not use this launcher for sampling sweeps.
EOF
  exit 4
fi

RUN_NAME=${LATENTFM_XVERSE_SAMPLING_RUN_NAME:-xverse_sampling_conservative_v2_power05_cap8_min32_4k_seed42}
RUN_ROOT=${LATENTFM_XVERSE_SAMPLING_RUN_ROOT:-${ROOT}/runs/latentfm_xverse_sampling_conservative_smoke_20260622}
OUT_ROOT=${LATENTFM_XVERSE_SAMPLING_OUT_ROOT:-${COUPLED}/output/latentfm_runs/xverse_sampling_conservative_smoke_20260622}
LOG_ROOT=${LATENTFM_XVERSE_SAMPLING_LOG_ROOT:-${ROOT}/logs/latentfm_xverse_sampling_conservative_smoke_20260622}
DATA_DIR=${ROOT}/dataset/latentfm_full/xverse
BIFLOW_DIR=${ROOT}/dataset/biFlow_data
CANONICAL_SPLIT=${ROOT}/dataset/biFlow_data/split_seed42.json
TRAINONLY_SPLIT=${LATENTFM_XVERSE_SAMPLING_TRAIN_SPLIT:-${ROOT}/dataset/biFlow_data/split_seed42_xverse_trainonly_crossbg_val_v2.json}
TRAINONLY_PERT_MEANS=${LATENTFM_XVERSE_SAMPLING_PERT_MEANS_FILE:-${ROOT}/runs/latentfm_xverse_trainonly_crossbg_val_20260622/artifacts/xverse_trainonly_pert_means_split_seed42_crossbgval_v2.npz}
...
session=${LATENTFM_XVERSE_SAMPLING_SESSION:-latentfm_xverse_sampling_cons_20260622}
mkdir -p "${run_root}/logs" "${run_root}/scripts" "${log_root}" "${OUT_ROOT}" "${ROOT}/reports"

if [[ -z "${TRAINONLY_PERT_MEANS}" || "${TRAINONLY_PERT_MEANS}" != *xverse_trainonly_pert_means* ]]; then
  echo "Refusing to launch: sampling smoke requires train-only xverse pert means. Got: ${TRAINONLY_PERT_MEANS}" >&2
  exit 4
fi
...
for required in \
  ...
  "${GPU_HELPER}" \
  "${TRAIN_LAUNCHER}" \
  "${ROOT}/reports/LATENTFM_XVERSE_TRAINONLY_V2_SAMPLING_CONSERVATIVE_CPU_GATE_20260622.md" \
...
if [[ -e "${out_dir}" && "${FORCE_XVERSE_SAMPLING_RERUN:-0}" != "1" ]]; then
  echo "Output exists for ${RUN_NAME}; set FORCE_XVERSE_SAMPLING_RERUN=1 to relaunch" >&2
  exit 3
fi
...
echo "[$(date '+%F %T %Z')] exact GPU status before conservative sampling launch" | tee "${run_root}/logs/gpu_launch_audit.log"
...
export SEED=${LATENTFM_XVERSE_SAMPLING_SEED:-42}
export INIT_CHECKPOINT=${ANCHOR_CKPT}
export FINETUNE_TRAINABLE_SCOPE=all
export TOTAL_STEPS=${LATENTFM_XVERSE_SAMPLING_TOTAL_STEPS:-4000}
...
export MIN_SELECTED_CONDITIONS_PER_DATASET=32
export CONDITION_VISIT_POWER=0.5
export CONDITION_VISIT_CAP=8
...
${PYTHON} ${ROOT}/ops/summarize_latentfm_single_background_candidate_decision_20260622.py --gate-json "\${eval_dir}/single_background_candidate_gate.json" --label ${RUN_NAME} --title "LatentFM xverse conservative sampling candidate decision" --out-md "\${eval_dir}/SINGLE_BACKGROUND_CANDIDATE_DECISION.md"
EOF
...
\`\`\`bash
LATENTFM_XVERSE_SAMPLING_ACK=cpu_gate_passed_single_candidate bash ${ROOT}/ops/launch_latentfm_xverse_sampling_conservative_smoke_20260622.sh
\`\`\`
...
Gated P3 conservative sampling smoke. Uses train-only v2 split and pert means,
warm-starts the xverse seed42 8k anchor, and changes only sampling exposure:
\`condition_visit_power=0.5\`, \`condition_visit_cap=8\`,
\`min_selected_conditions_per_dataset=32\`. No residual, condition-delta,
anchor-replay, condition-source, or architecture changes are enabled.
EOF
