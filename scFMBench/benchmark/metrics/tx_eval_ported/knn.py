...
class WeightedKNNClassifier(Metric):
    ...
    def update(
        ...
        # Convert labels if they are not indices (Tensor = already encoded)
        if train_targets is not None and not isinstance(train_targets, torch.Tensor):
            first = train_targets.iloc[0] if hasattr(train_targets, "iloc") else train_targets[0]
            if isinstance(first, str):
                ts = train_targets if hasattr(train_targets, "iloc") else pd.Series(train_targets)
                train_targets = self._extract_labels(ts)

        if test_targets is not None and not isinstance(test_targets, torch.Tensor):
            first = test_targets.iloc[0] if hasattr(test_targets, "iloc") else test_targets[0]
            if isinstance(first, str):
                ts = test_targets if hasattr(test_targets, "iloc") else pd.Series(test_targets)
                test_targets = self._extract_labels(ts)
